import { AppEvent, isMeetingEvent, isTaskEvent, isReminderEvent } from '../models/event.model.js';

// Generic function to find element by id (with generic constraint)
export function findEventById<T extends AppEvent>(
    events: T[],
    id: string
): T | undefined {
...
export function getEventProperty<T extends AppEvent, K extends keyof T>(
    event: T,
    property: K
): T[K] {
...
export function formatEventDetails(event: AppEvent): string {
...
export function batchProcessEvents<T>(
    events: AppEvent[],
    processor: (event: AppEvent) => T
): T[] {
    return events.map(processor);
}

// Sort events by date
export function sortEventsByDate(events: AppEvent[]): AppEvent[] {
